<template>
  ...
  export default {
    ...
    computed: {
      favoriteIcon() {
        // お気に入りアイコンのクラスを返す
        return this.favorites.includes(this.productId) ? 'fa-solid fa-heart' : 'fa-regular fa-heart';
      },
    },
    methods: {
      async toggleFavorite() {
      console.log('Inside toggleFavorite, plantId:', this.productId);
      // お気に入りアイコンがクリックされたときの処理
      if (this.favorites.includes(this.productId)) {
        // すでにお気に入りに登録されている場合は削除する
        this.favorites = this.favorites.filter((id) => id !== this.productId);
      } else {
        // お気に入りに登録されていない場合は追加する
        this.favorites.push(this.productId);
        const dataToSend = { productId: this.productId, email: this.userEmail };
        console.log('Sending request with data:', dataToSend); // ここでログ出力
        // サーバーにお気に入り登録のリクエストを送信
        try {
          await this.addToFavorites(this.productId);
      ...
      return axios.post('/api/add-to-favorites-product', { productId, email: this.userEmail });
    },
    },
    mounted() {
